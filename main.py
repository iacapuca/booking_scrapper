...
def number_of_pages(page):
    pagination = page.locator('[data-testid="pagination"] nav').all_inner_texts()
...
def run(p):
    ...
    for i in range(1, pages + 1):
        ...
        for property_card in property_cards:
            ...
            hotel_link = property_card.get_by_test_id("title-link").get_attribute(
                "href"
            )
            hotels_info.append({"name": hotel_name, "link": hotel_link})
        if i != pages:
